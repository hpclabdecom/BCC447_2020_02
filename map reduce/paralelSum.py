...
class OperationParalel:
    ...
    def run(self, values, high = 0):
        if len(values) <= 2 or 2**high > self.threadCounter:
            return self.operation(values)
        else:
            if not self.executor:
                self.executor = concurrent.futures.ThreadPoolExecutor(self.threadCounter)
                
            values_left = values[:len(values) // 2]
            values_right = values[len(values) // 2:]

            # chamadas recusivas, esquerda e direita
            left_future = self.executor.submit(self.run, values_left, high + 1)
            right_future = self.executor.submit(self.run, values_right, high + 1)
...
class OperationParalelIter:
    def __init__(self, operation, thread_count = os.cpu_count()):
        self.operation = operation
